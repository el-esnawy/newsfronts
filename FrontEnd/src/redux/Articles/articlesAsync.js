  ...
  async (arg) => {
    ...
    if (arg.searchterm) {
      const response = await axios.get(
        `https://newsfronts.herokuapp.com/api/v1/search/${arg.searchterm}`,
      );

      return { ...response.data, responseURL: response.request.responseURL };
    } else if (arg.category === "headlines") {
      const response = await axios.get(
        "https://newsfronts.herokuapp.com/api/v1/headlines",
        {
          params: {
            pageNumber: page || 1,
      ...
      const response = await axios.get(
        `https://newsfronts.herokuapp.com/api/v1/${Object.keys(arg)[0]}/${
          Object.values(arg)[0]
        }`,
        {
          params: {
